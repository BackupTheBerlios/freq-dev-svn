...
def load_plugins():
	valid_plugins = find_plugins(); fp=file("pg_plugin.py"); exec fp in globals(); fp.close();
...
def has_access(source, required_level):
	jid = get_true_jid(source)
	if 2*new_access(source[1]+u"/"+source[2]) >= required_level:
		return 1
	return 0
...
def join_groupchat(groupchat, nick=None):
	x={};
	x=load_info("dynamic/muc_status");
	if nick:
		set_nick(groupchat, nick)
	else:
		nick=get_nick(groupchat);

	JCON.send(xmpp.Presence(groupchat + '/' + nick, show=x["show"], status=x["status"]))
...
def messageCB(con, msg):
	msgtype = msg.getType(); body="";
	if msg.getBody(): body=rewrite(rewrite(msg.getBody().strip(), msg.getFrom()), "global");
	fromjid = msg.getFrom()
	command = ''
	parameters = ''
...
def presenceCB(con, prs):
	call_presence_handlers(prs); debug_prs=prs;
	type = prs.getType()
	...
	if groupchat in GROUPCHATS:
		if type == 'available' or type == None:
			if not GROUPCHATS[groupchat].has_key(nick):
				GROUPCHATS[groupchat][nick] = {'jid': prs.getFrom(), 'idle': time.time(), 'affiliation': prs.getAffiliation(), 'role': prs.getRole()}; PRS["%s/%s" % (groupchat, nick)]=prs;
...
def start():
	...
	thread.start_new(joinall, (groupchats, ))
