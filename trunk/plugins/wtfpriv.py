...
def fwtfpriv(type,source,body):
 ...
 if slf or has_access(source,2*toint(configget(r,"showprivaccess"),12)):
  if GROUPCHATS[r].has_key(n): msg(r+u"/"+n, arr2str([i[0] for i in db.get("select d from vocabulary where (r='g' or r=?) and w=? order by length(r) desc, t desc limit 1", (r, w))])); s="sent";
  else: s=u"кому?"
