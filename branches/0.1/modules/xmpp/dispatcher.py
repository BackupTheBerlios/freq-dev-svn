...
class Dispatcher(PlugIn):
    ...
    def Process(self, timeout=0):
        ...
        if self._owner.Connection.pending_data(timeout):
            ...
            #self.Stream.pginit();
            #try: self.Stream.Parse2(("<stream>"+data,"")[int( bool(data.count("<"+"?")))])
            #except: f=file("/bad","w"); import sys;p=sys.exc_info();s="%s:\n%s %s %s\n" % (time.strftime("%H:%M:%S\n"), p[0], p[1], p[2].tb_frame.f_code);f.write(s);f.write(data);f.close();data=data;
            self.Stream.pgp.Parse(data);
            if len(self._pendingExceptions) > 0:
                _pendingException = self._pendingExceptions.pop()
                raise _pendingException[0], _pendingException[1], _pendingException[2]
